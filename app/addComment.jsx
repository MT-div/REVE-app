...
const addComment = () => {
  ...
  // and provides separate  alerts based on missing inputs.
  const handleSubmit = async () => {
    ...
    if (!comment.trim() && rating === 0) {
      Alert.alert('تنبيه', 'يرجى كتابة التعليق وتحديد التقييم');
      return;
    }
    try {
  ...
  return (
    <View style={styles.container}>
      <View style={styles.topHolder}>
        <TouchableOpacity onPress={() => navigation.replace("CardDitals", { id })}>
