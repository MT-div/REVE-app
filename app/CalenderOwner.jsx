...
const Calendar = () => {
  ...
  const handleDayPress = (day) => {
    if (isPast(day) || isReserved(day)) return;
    if (!selectedRange.start || selectedRange.end) {
      setSelectedRange({ start: day, end: null });
    } else if (day.isSameOrAfter(selectedRange.start)) {
  ...
  const renderDay = ({ item: day }) => {
    ...
    if (isActive) {
      ...
      if (isPast(day)) {
        ...
        if (reservation.status === 'accepted' || reservation.status === 'DayOff') {
          reservation.isMine 
            ? textStyles.push(styles.myReservation)
            : textStyles.push(styles.bookedDay);
    ...
    const disabled = isPast(day) || !!reservation;
