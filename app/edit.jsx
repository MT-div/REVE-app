...
const updateProfileInfo = async (userData) => {
  try {
...
// Dropdown options for governorates as provided
...
const SignupScreen = () => {
  ...
  // Form state. This gets prefilled with a GET request.
  ...
  // Fetch current profile data to prefill the fields
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        setFormData(prev => ({
          ...
          fullName: data.name || '',
          phone: data.phone || '',
          location: data.city || '', // prefill with city if available
  ...
  const validateFirstSection = () => {
    const newErrors = {
      ...
      // Validate password only if the user enters a new value.
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Build payload – if password fields are empty, backend may ignore updates.
      const userData = {
        username: formData.username,
        password: formData.password,
        confirm_password: formData.confirmPassword,
        phone: formData.phone,
        city: formData.location, // using location field for the governorate
      ...
      Alert.alert('فشل التعديل', errorMessage);
    }
  };
